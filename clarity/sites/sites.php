...
$sqlappCount = "select count(1) total from sites a where 1 ";
...
if (isset($_REQUEST['isDone']) && $_REQUEST['isDone'] != '') {
    $isDone = $_REQUEST['isDone'];
    $atm_sql .= "and a.isDone like '%" . $isDone . "%'";
    $sqlappCount .= "and a.isDone like '%" . $isDone . "%'";
}
            ...
            <div class="card-body">
                <form id="sitesForm" action="<?php echo basename(__FILE__); ?>" method="POST">
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <label>Live Sites</label>
                            <select name="isDone" class="form-control">
                                <option value="">Select</option>
                                <option value="1" <? if (isset($isDone) && $isDone == 1) {
                                    echo 'selected';
                                } ?>>Yes</option>
                                <option value="0" <? if (isset($isDone) && $isDone == 0) {
                                    echo 'selected';
                                } ?>>No</option>
                            </select>
                ...
                <table class="table dataTable js-exportable no-footer table-xs" style="width:100%;">
                    <thead>
                        <tr class="table-primary">
                            <th>#</th>
                            <th class="readonlyAccess" style="padding-right:5px;">
                    ...
                    <tbody>
                        ...
                        while ($atm_sql_result = mysqli_fetch_assoc($atm_sql_res)) {
                            ...
                            $projectInstallationsql = mysqli_query($con, "select * from projectInstallation where siteid = '" . $id . "' and status=1 and isDone=1");
                            ...
                            <tr <? if ($projectInstallation) { ?> style="background:#4CAF50;color:white;" <? } ?>>
                                <th>
                                    <?php echo $counter; ?>
                                </th>
                                <td class="readonlyAccess">
                                    <?
                                ...
                                <td class="strong" style="    font-weight: 700;">
                                    <a class="readonlyAccess"
                                        href="atmEdit.php?id=<? echo $id; ?>&atmid=<? echo $atmid; ?>"><i
                                            class="mdi mdi-tooltip-edit" style="
    font-size: 20px;
    color: green;
    cursor: pointer;
...
"></i>|</a>
                                ...
                                <td>
                                    ...
                                    if ($projectInstallation) {
                                        ...
                                        if ($isFeasibiltyDoneRecord) {
                                            ...
                                            if ($isDelegated == 0) {
                                                echo '<a class="readonlyAccess" href="singleDelegate.php?id=' . $id . '&atmid=' . $atmid . '">Delegate ➜</a>';
                                            } else {
                                                echo '<button class="btn btn-success btn-icon" style="  width: 20px;height: auto !important;">&#10004;</button> 
            | 
            <a class="readonlyAccess" href="delegate.php?id=' . $id . '&atmid=' . $atmid . '&action=redelegate">Redelegate <span style="color:red">⟳</span></a>
            |
            ...
            ';
                                ...
                                <td>
                                    ...
                                    <a href="#" data-bs-toggle="modal" class="btn btn-primary history-link"
                                        data-bs-target="#historyModal" data-act="add"
                                    ...
                                    if ($projectInstallation) {
                                        ?>
                                        |
                                        <a href="#" data-bs-toggle="modal" class="btn btn-secondry schedule-link"
                                            data-bs-target="#scheduleModal" data-act="add" data-siteid="<?php echo $id; ?>">View
                                            Schedule</a>
                                    <?php }
                                    ?>
                                ...
                                <td>
                                    ...
                                    if ($projectInstallation) {
                                        ...
                                        <a href="schedule.php?id=<?= $id; ?>&atmid=<?= $atmid; ?>"
                                            class="btn btn-primary readonlyAccess">Schedule</a>
                                        <?
            ...
            if ($start_window > 1) {
                ...
                echo "<li class='paginate_button'><a href='?page=1&&atmid=$atmid&&$customer&&page_size=$page_size&&isDelegated=$    &&isFeasibiltyDone=$isFeasibiltyDone&vendorId=$vendorId'&state='.$state.'&isDone='.$isDone.'>First</a></li>";
                echo '<li class="paginate_button"><a href="?page=' . ($start_window - 1) . '&&atmid=' . $atmid . '&&customer=' . $customer . '&&page_size=' . $page_size . '&&isDelegated=' . $isDelegated . '&&isFeasibiltyDone=' . $isFeasibiltyDone . '&vendorId=' . $vendorId . '&state=' . $state . '&isDone=' . $isDone . '">Prev</a></li>';
            }
            ...
            for ($i = $start_window; $i <= $end_window; $i++) {
                ...
                <li class="paginate_button <? if ($i == $current_page) {
                    ...
                    <a
                        href="?page=<? echo $i; ?>&&atmid=<? echo $atmid; ?>&&customer=<? echo $customer; ?>&&page_size=<? echo $page_size; ?>&&isDelegated=<? echo $isDelegated; ?>&&isFeasibiltyDone=<? echo $isFeasibiltyDone; ?>&vendorId=<?= $vendorId; ?>&state=<?= $state; ?>&isDone=<?= $isDone; ?>">
                        <? echo $i; ?>
                    </a>
                </li>
            ...
            if ($end_window < $total_pages) {
                ...
                echo '<li class="paginate_button"><a href="?page=' . ($end_window + 1) . '&&atmid=' . $atmid . '&&customer=' . $customer . '&&page_size=' . $page_size . '&&isDelegated=' . $isDelegated . '&&isFeasibiltyDone=' . $isFeasibiltyDone . '&vendorId=' . $vendorId . '&state=' . $state . '&isDone=' . $isDone . '">Next</a></li>';
                echo '<li class="paginate_button"><a href="?page=' . $total_pages . '&&atmid=' . $atmid . '&&customer' . $customer . '&&page_size=' . $page_size . '&&isDelegated=' . $isDelegated . '&&isFeasibiltyDone=' . $isFeasibiltyDone . '&vendorId=' . $vendorId . '&state=' . $state . '&isDone=' . $isDone . '">Last</a></li>';
            }
            echo '</ul></div></div>';
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="ModalLabel">ATM History</h5>
