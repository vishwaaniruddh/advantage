...
<?
    ...
    ?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <select name="isDelivered" id="" class="form-control">
                                            ...
                                            <option value="1" <? if ($_REQUEST['isDelivered'] == 1) {
                                                echo 'selected';
                                            } ?>>Delivered
                                            </option>
                                            <option value="0" <? if ($_REQUEST['isDelivered'] == 0) {
                                                echo 'selected';
                                            } ?>>In-Transit
                                            </option>
            ...
            $atm_sql = "SELECT ms.id,ms.atmid, msd.serialNumber AS latest_serialNumber,ms.contactPersonName,
            ms.address,ms.pod,ms.courier,ms.lho,ms.siteid,ms.vendorId,ms.contactPersonNumber,ms.remark,ms.created_at
            
            FROM material_send ms JOIN material_send_details msd 
            ON ms.id = msd.materialSendId WHERE msd.attribute = 'Router' and ms.portal='clarity' AND 
            ...
            $atm_sql .= "  and ms.portal='clarity' group by latest_serialNumber";
            ...
            ?>
                            ...
                            <table class="table table-hover table-styling table-xs">
                                ...
                                while ($sql_result = mysqli_fetch_assoc($sql_app)) {
                                    ...
                                    if (!in_array($router_serial_number, $serial_number_ar)) {
                                        ...
                                        echo "<tr class='clickable-row' data-bs-toggle='collapse' data-bs-target='#details-$id'>";
                                        echo "<td>$counter</td>";
                ...
                <div class="dataTables_paginate paging_simple_numbers" id="example_paginate"><ul class="pagination">';
                    ...
                    if ($start_window > 1) {
                        ...
                        echo "<li class='paginate_button'><a href='?page=1&&atmid=$atmid&&isDelivered=$isDelivered'>First</a></li>";
                        echo '<li class="paginate_button"><a href="?page=' . ($start_window - 1) . '&&atmid=' . $atmid . '&isDelivered=' . $isDelivered . '">Prev</a></li>';
                    }
                    ...
                    for ($i = $start_window; $i <= $end_window; $i++) {
                        ?>
                                <li class="paginate_button <? if ($i == $current_page) {
                                    ...
                                    <a href="?page=<?= $i; ?>&&atmid=<?= $atmid; ?>&&isDelivered=<?= $isDelivered; ?>">
                                <?= $i; ?>
                                    </a>
                                </li>
                    ...
                    if ($end_window < $total_pages) {
                        ...
                        echo '<li class="paginate_button"><a href="?page=' . ($end_window + 1) . '&&atmid=' . $atmid . '&isDelivered=' . $isDelivered . '">Next</a></li>';
                        echo '<li class="paginate_button"><a href="?page=' . $total_pages . '&&atmid=' . $atmid . '&isDelivered=' . $isDelivered . '">Last</a></li>';
                    }
                    echo '</ul></div></div>';
