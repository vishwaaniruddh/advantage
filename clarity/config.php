...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO datarecords (requestData, sessionData, fileData, created_at, created_by) 
            VALUES (?, ?, ?, ?, ?)";
    ...
    if ($stmt = mysqli_prepare($con, $sql)) {
        ...
        mysqli_stmt_bind_param($stmt, 'ssssi', $requestData, $sessionData, $fileData, $datetime, $userid);

        // Set the $datetime and $userid variables
        $datetime = date('Y-m-d H:i:s');  // Current datetime
        $userid = $_SESSION['ADVANTAGE_userid'];  // Example of getting the user ID from the session
...
{
    global $con;
    // echo "select * from material_requests where siteid='" . $siteid . "' and isProject=1 order by id desc";
    $sql = mysqli_query($con, "select * from material_requests where siteid='" . $siteid . "' and isProject=1 order by id desc");
    $sql_result = mysqli_fetch_assoc($sql);
...
function getInstallationStatusWithRemark($imageType, $installationID)
{
    global $con;
    $remark = '';
    $result = [];

    // echo "select * from installationcheckpoints where installationID='" . $installationID . "' and isApproved" . $imageType . " in(1,2)" ;
    $installationStatusSql = mysqli_query($con, "select * from installationcheckpoints where installationID='" . $installationID . "' and isApproved" . $imageType . " in(1,2)");
    if ($installationStatusSqlResult = mysqli_fetch_assoc($installationStatusSql)) {

        $verificationStatus = $installationStatusSqlResult['isApproved' . $imageType];
        $verifyby = $installationStatusSqlResult[$imageType . 'ApprovedBy'];

        // echo "select remark from installationcheck_checkpoints_remarks where installationID ='".$installationID."' and fieldName like 'isApproved" . $imageType . "' and status=1 order by id desc" ; 
        $remark_sql = mysqli_query($con, "select remark from installationcheck_checkpoints_remarks where installationID ='" . $installationID . "' and fieldName like 'isApproved" . $imageType . "' and status=1 order by id desc");
        
        if($remarksqlResult = mysqli_fetch_assoc($remark_sql)){
            $remark = $remarksqlResult['remark'];
        }
        

        $result = ['response' => 200, 'status' => $verificationStatus, 'verifyby' => $verifyby, 'remark' => $remark];
